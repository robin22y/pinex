...
async function callGemini(prompt) {
  ...
  return new Promise((resolve, reject) => {
    const path = `/v1beta/models/gemini-3.1-flash:generateContent?key=${apiKey}`
    const req = https.request(
      {
...
async function saveDescription(symbol, description) {
  ...
  const body = JSON.stringify({ description, description_approved: false, updated_at: new Date().toISOString() })
