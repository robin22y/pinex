...
const parseSmartQuery = (query, allStocks, market) => {
  ...
  if (q.length >= 2) {
    // 2. SYMBOL-PREFIX match — strong signal of stock intent (user typing a ticker).
    //    Run before sector check so "hdfc" → HDFCBANK/HDFCAMC, not Banking sector.
    const nameExact = (s) => s.name?.toLowerCase() === q
    const symStart  = (s) => s.symbol?.toLowerCase().startsWith(q)
    const symPrefixMatches = allStocks
      .filter(symStart)
      .sort((a, b) => {
        if (nameExact(a) && !nameExact(b)) return -1
        if (!nameExact(a) && nameExact(b)) return 1
        return (b.rs_vs_nifty || -999) - (a.rs_vs_nifty || -999)
      })
      .slice(0, 50)
    if (symPrefixMatches.length === 1) return { type: 'stock', stock: symPrefixMatches[0] }
    if (symPrefixMatches.length > 1) return { type: 'stock_list', stocks: symPrefixMatches, label: `Stocks matching "${query}"` }
  }
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const load = async (background = false) => {
      ...
      try {
        const [
          { data: homeStocks, error: homeStocksErr },
          { data: mkt },
          { data: mktHistory },
          { data: sec },
        ] = await Promise.all([
          withTimeout(supabase.rpc('get_home_stocks')),
        ...
        if (rpcBatch.length < FULL_UNIVERSE_THRESHOLD || homeStocksErr) {
          ...
          console.warn(
            ...
            `Falling back to companies + price_data ` +
