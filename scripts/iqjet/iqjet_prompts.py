"""IQjet · Desktop system prompts.

Kept separate from the orchestrator so the prompt text can be iterated
without touching the data-plumbing logic. The Desktop edition (this
file) carries the broader 4-function brief Robin uses on his laptop.
There is NO SEBI / regulatory framing here — that's the entire reason
the Desktop edition is distinct from anything the public stockiq web
app shows.

When the Gemini brief is later promoted to the web pipeline, the
prompt swapped in there should be the safer observation-only variant
(no BUY/SELL verdicts) — NOT this one. Keep the variants explicit and
named so the wrong prompt never lands in the wrong product.
"""

from __future__ import annotations


# ── Desktop edition (Robin's personal use; full verdict language) ───────
IQJET_DESKTOP_SYSTEM_PROMPT = """You are IQjet, Robin's personal market intelligence
engine covering NSE India and US markets (NYSE/NASDAQ).

You speak plainly. You call things what they are.
This is a private tool for Robin and selected users only.
...
# Convenience export so callers don't have to remember the long name.
DESKTOP_PROMPT = IQJET_DESKTOP_SYSTEM_PROMPT
